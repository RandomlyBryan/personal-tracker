            ...
            if days_since >= get_days_interval(row.get('frequency', 'Daily')):
                ...
                with col_text:
                    ...
                    if row.get('task_id') not in st.session_state.emails_sent_today:
                        if send_email_notification(row.get('task_name', 'Task'), days_since, row.get('task_description', ''), row.get('task_url', '')):
                ...
                with col_action:
                    ...
                    with col_btn:
                        if st.button("Done", key=f"remind_btn_{row.get('task_id')}", use_container_width=True):
                            ...
                            if str(row.get('is_recurring', 'Yes')) == "No": df = df[df['task_id'] != row.get('task_id')]
                            else: df.at[index, 'last_completed'] = today.strftime(STORAGE_DATE_FORMAT)
                            save_and_push(df, DB_FILE)
                            if row.get('task_id') in st.session_state.emails_sent_today: st.session_state.emails_sent_today.remove(row.get('task_id'))
                            st.rerun()
        ...
        with eod_log_col:
            ...
            with st.form("eod_add_form", clear_on_submit=True):
                ...
                if st.form_submit_button("Add") and log_input:
                    new_log_id = int(eod_df['log_id'].max() + 1) if not eod_df.empty else 1
                    new_log_row = {"log_id": new_log_id, "task_title": manual_title.strip() if manual_title.strip() else "Manual Log", "bullet_text": log_input.strip(), "log_date": today.strftime(STORAGE_DATE_FORMAT), "task_links": "", "screenshot_b64": ""}
                    eod_df = pd.concat([eod_df, pd.DataFrame([new_log_row])], ignore_index=True)
                    save_and_push(eod_df, EOD_FILE)
                    st.rerun()
        with prio_log_col:
        ...
        with sub_tab_task:
            with st.form("new_task_form", clear_on_submit=True):
                ...
                if st.form_submit_button("Save Routine") and new_name:
                    ...
                    df = pd.concat([df, pd.DataFrame([{"task_id": new_id, "task_name": new_name, "task_description": new_desc if new_desc else "No instructions.", "task_url": comma_links, "frequency": new_freq, "is_recurring": recurrence_setting, "last_completed": start_date.strftime(STORAGE_DATE_FORMAT), "task_screenshot_b64": media_b64}])], ignore_index=True)
                    save_and_push(df, DB_FILE)
                    st.rerun()
        with sub_tab_note:
        ...
        with m_task:
            for index, row in df.iterrows():
                ...
                if st.session_state.editing_task_id == row.get('task_id'):
                    ...
                    with ec1:
                        st.write(f"**{row.get('task_name', 'Task')}** ({row.get('frequency', 'Daily')} — *{'One-Time' if str(row.get('is_recurring', 'Yes')) == 'No' else 'Recurring'}*)")
